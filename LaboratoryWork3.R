  ...
  for(k in 1:ncol(values)){
    for(i in 1:10**k){
      for(j in 1:nrow(values)){
        matrixY[j,k] <<- matrixW[,k] %*% unlist(matrixResult[j,], use.names = FALSE)
        matrixW[,k] <<- matrixW[,k] + (1/nrow(values)) * matrixY[j,k] * (unlist(matrixResult[j,], use.names = FALSE) - matrixY[j,k] * matrixW[,k])
        matrixW[,k] <<- matrixW[,k]/normVector(matrixW[,k])
      }
    }
    matrixResult <- matrixResult - matrixY[,k] %*% t(matrixW[,k])
  }
  return(matrixResult)
}
